...
def sentence3() -> Expr:
    ...
    exp1 = PacmanAlive_1 % disjoin(conjoin(PacmanAlive_0, ~PacmanKilled_0), conjoin(~PacmanAlive_0, PacmanBorn_0))
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    action = []
    for a in DIRECTIONS:
        action.append(PropSymbolExpr(a, time = t))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    condi = conjoin(KB)
    all_condi_1 = conjoin(condi, PropSymbolExpr(pacman_str, x1, y1, time = 1))
    all_condi_2 = conjoin(condi, ~PropSymbolExpr(pacman_str, x1, y1, time = 1))
    model1 = findModel(all_condi_1)
    model2 = findModel(all_condi_2)
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        condi = conjoin(KB)
        assign = findModel(conjoin(PropSymbolExpr(pacman_str, xg, yg, time = t), condi))
        if assign:
            return extractActionSequence(assign, actions)
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        foodL = []
        for x, y in food:
            foodL.append(PropSymbolExpr(food_str, x, y, time = t))
        for x, y in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(x, y, time = t+1, walls_grid = walls))
        for x, y in all_coords:
            KB.append(disjoin(PropSymbolExpr(pacman_str, x, y, time = t), ~PropSymbolExpr(food_str, x, y, time = t)) % ~PropSymbolExpr(food_str, x, y, time = t+1))
        goal = ~disjoin(foodL)
        condi = conjoin(KB)
        assign = findModel(conjoin(goal, condi))
        if assign:
            return extractActionSequence(assign, actions)
    "*** END YOUR CODE HERE ***"
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            ...
            if findModel(condi & pac):
                possible_locations.append((x,y))
            if entails(condi, pac):
                KB.append(pac)
            elif entails(condi, ~pac):
                KB.append(~pac)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            ...
            if entails(condi, wall):
                KB.append(wall)
                known_map[x][y] = 1
            elif entails(condi, ~wall):
                KB.append(~wall)
                known_map[x][y] = 0
        agent.moveToNextState(agent.actions[t])
        "*** END YOUR CODE HERE ***"
