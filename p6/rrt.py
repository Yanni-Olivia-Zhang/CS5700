...
class RRTPlannerPolygon:
    ...
    def plan(self, start, goal):
        ...
        while not goal_found:
            ...
            confi = self.sample_configuration()
            neighbor = self.graph.nearest(confi)
            clipped = self.make_clipped_node(neighbor, confi)
            if self.is_connectable(neighbor, clipped):
                self.graph.add_node(clipped)
                self.graph.add_edge(neighbor, clipped)
                if self.rrt_star == True:
                    self.rewire_node(self.graph, clipped)
                goal_found = self.try_connect(self.graph, clipped, n_goal)
    ...
    def rewire_node(self, graph, n_tgt):
        ...
        nodes = graph.nodes
        for n in nodes:
            connect = self.is_connectable(n, n_tgt)
            dis = n.pose.get_linear_distance(n_tgt.pose)
            if connect and (dis <= self.rewire_radius):
                oldcost = n_tgt.cost
                newcost = n.cost + dis
                if newcost < oldcost:
                    n_rewire = n
        ...
        if n_rewire is not None:
            "*** BEGIN YOUR CODE HERE ***"
            #raiseNotDefined()
            graph.remove_edge(n_tgt.parent, n_tgt)
            graph.add_edge(n_rewire, n_tgt)
            n_tgt.parent = n_rewire
            n_tgt.cost = n_rewire.pose.get_linear_distance(n_tgt.pose)
            "*** END YOUR CODE HERE ***"
            self.n_rewires += 1
    ...
    def try_connect(self, graph, n_curr, n_tgt):
        ...
        while True:
            ...
            if self.rrt_connect:
                "*** BEGIN YOUR CODE HERE ***" 
                #raiseNotDefined()
                n_new = self.make_clipped_node(n_curr, n_tgt.pose)
                if self.is_connectable(n_curr, n_new):
                    n_new.parent = n_curr
                    graph.add_node(n_new)
                    graph.add_edge(n_curr, n_new)
                    n_curr = n_new
                else:
                    return False
                "*** END YOUR CODE HERE ***"
