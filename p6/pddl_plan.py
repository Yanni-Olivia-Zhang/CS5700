...
def create_action_transit():
    ...
    transit = Action(
        name = 'transit',
        args = ['?robot', '?from_room', '?to_room'],
        types=['robot', 'room', 'room'],
        pre_condition = [('+', Fluent('robot_in_room', '?robot', '?from_room')),
                         ('+', Fluent('adjacent', '?from_room', '?to_room')),
                         ('-', Fluent('robot_in_room', '?robot', '?to_room')),
                         ('-', Fluent('oriented', '?robot')),
                        ],                  
        post_condition = [('+', Fluent('robot_in_room', '?robot', '?to_room')),
                          ('-', Fluent('robot_in_room', '?robot', '?from_room')),
                         ],
    )
    return transit
...
def create_action_recenter():
    ...
    recenter = Action(
        name = 'recenter',
        args = ['?robot', '?spawn', '?room'],
        types=['robot', 'spawn', 'room'],
        pre_condition = [('+', Fluent('oriented', '?robot')),
                         ('+', Fluent('robot_in_room', '?robot', '?room')),
                         ('+', Fluent('spawn_in_room', '?spawn', '?room')),
                         ('+', Fluent('at_spawn', '?robot', '?spawn')),
                        ],
        post_condition = [('-', Fluent('oriented', '?robot')),
                          ('-', Fluent('at_spawn', '?robot', '?spawn')),
                         ],
    )
    return recenter
...
def create_action_pick():
    ...
    pick = Action(
        name = 'pick',
        args = ['?robot', '?object', '?spawn'],
        types=['robot', 'object', 'spawn'],
        pre_condition = [('+', Fluent('at_spawn', '?robot', '?spawn')),
                         ('+', Fluent('at_spawn', '?object', '?spawn')),
                         ('+', Fluent('empty', '?robot')),
                        ],
        post_condition = [('+', Fluent('holding', '?object', '?robot')),
                          ('-', Fluent('at_spawn', '?object', '?spawn')),
                          ('-', Fluent('empty', '?robot')),
                         ],
    )
...
def create_action_place():
    ...
    place = Action(
        ...
        args = ['?robot', '?object', '?spawn'],
        types=['robot', 'object', 'spawn'],
        pre_condition = [('+', Fluent('holding', '?object', '?robot')),
                         ('+', Fluent('at_spawn', '?robot', '?spawn')),
                        ],
        post_condition = [('+', Fluent('at_spawn', '?object', '?spawn')),
                          ('+', Fluent('empty', '?robot')),
                          ('-', Fluent('holding', '?object', '?robot')),
                         ],
    )
    return place
    "*** END YOUR CODE HERE ***"
